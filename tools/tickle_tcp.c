    ...
    if (have_allocations)
    {
        ...
        do
        {
            ...
            if (fgets_rc != NULL)
            {
                ...
                trim_left(line_buffer, strlen(line_buffer));
                trim_right(line_buffer, strlen(line_buffer));

                bool is_ipv6 = false;
                const bool have_endpoints = parse_endpoints(
                    line_buffer, strlen(line_buffer), &buffers, &endpoint_data, &is_ipv6, &error
                );
                if (have_endpoints)
                {
                    bool send_rc = false;
...
)
{
    ...
    {
        error->error_msg = "Warning: Ignored invalid input line";
        error->error_nr = 0;
    }
...
)
{
    ...
    if (packet != NULL)
    {
        ...
        // ip_header->ip6_plen = htons(20);
        // ip_header->ip6_nxt  = IPPROTO_TCP;
        // ip_header->ip6_hlim = 64;
        // ip_header->ip6_src  = src_address->sin6_addr;
        // ip_header->ip6_dst  = dst_address->sin6_addr;
...
)
{
    ...
    if (socket_dsc != -1)
    {
        ...
        {
            bool socket_ready = socket_domain == AF_INET6;
            if (!socket_ready)
            {
                // Change socket options for inclusion of the custom IP header
                const uint32_t sockopt_value = 1;
                socket_ready = setsockopt(socket_dsc, SOL_IP, IP_HDRINCL, &sockopt_value, sizeof (sockopt_value)) == 0;
            }
...
void trim_left(char *const buffer, const size_t length)
{
    if (length >= 1)
    {
        size_t src_idx = 0;
...
void trim_right(char *const buffer, const size_t length)
{
    if (length >= 1)
    {
        size_t idx = length;
...
void close_file_dsc(const int file_dsc)
{
    int rc;
    do
    {
